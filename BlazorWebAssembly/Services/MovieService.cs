...
namespace MovieSearchBlazorApp.Services
{
    public class MovieService
    {
        ...
        {
            var response = await _httpClient.GetFromJsonAsync<ApiResponse>($"https://www.omdbapi.com/?s={query}&apikey={ApiKey}");
            return response?.Search;
        }

        public async Task<MovieDetail> GetMovieDetailAsync(string imdbID)
        {
            return await _httpClient.GetFromJsonAsync<MovieDetail>($"https://www.omdbapi.com/?i={imdbID}&apikey={ApiKey}");
        }
